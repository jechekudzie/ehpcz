            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0" id="page-title">System Roles</h4>
                ...
                <div class="row">
                                ...
                                <table style="width: 100%;" id="buttons-datatables"
                                    ...
                                    @foreach($roles as $role)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('admin.permissions.assign', $role->id) }}"
                    ...
                    <div class="col-xxl-3">
                        <div class="card border card-border-light">
                            <div class="card-header">
                                <h6 id="card-title" class="card-title mb-0">Add System Roles</h6>
                            </div>
                            <div class="card-body">
                                <form id="edit-form" action="{{route('roles.store')}}" method="post"
                                      enctype="multipart/form-data">
                                    <input type="hidden" name="_method" value="POST">
                                    @csrf
                                    <div class="mb-3">
                                        <label for="name" class="form-label">System Role</label>
                                        <input type="text" name="name" class="form-control" id="name"
                                               placeholder="Enter Module Name" value="">
                                    </div>
                                    <div class="text-end">
                                        <button id="submit-button" type="submit" class="btn btn-primary">Add New Role
                                        </button>
                                    </div>
                                </form>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('.edit-button').on('click', function () {
                ...
                $('#edit-form').attr('action', '/admin/roles/' + id );
                $('input[name="_method"]').val('PATCH');
                submitButton.text('Update');
                // Populate the form for editing
                $('#name').val(name);
                $('#card-title').text('Edit - ' + name + ' System Roles');
                $('#page-title').text('Edit - ' + name + ' System Roles');
            });

            // Click event for adding a new item
            $('#new-button').on('click', function () {
                // Clear the form, set action for creation, method to POST, and button text to Add New
                $('#edit-form').attr('action', 'admin/roles/store');
                $('input[name="_method"]').val('POST');
                submitButton.text('Add New');
                $('#name').val('');
                $('#card-title').text('Add System Roles');
                $('#page-title').text('Add New System Roles');
            });
        });
