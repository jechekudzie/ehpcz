            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0" id="page-title">Address Types</h4>
                ...
                <div class="row">
                            ...
                            <div class="card-header">
                                <div class="d-flex align-items-center flex-wrap gap-2">
                                    <div class="flex-grow-1">
                                        ...
                                        <a href="{{route('address-types.index')}}" class="btn btn-info btn-sm add-btn">
                                ...
                                <table style="width: 100%;" id="buttons-datatables"
                                    ...
                                    @foreach($addressTypes as $addressType)
                                        <tr class="even">
                                            ...
                                            <td>
                                                <!-- Edit Button -->
                                                <a href="javascript:void(0);" class="edit-button btn btn-sm btn-primary" data-name="{{ $addressType->name }}" data-slug="{{ $addressType->slug }}" title="Edit">
                                                    <i class="fa fa-pencil"></i>
                                                </a>

                                                <!-- Delete Button -->
                                                <form action="{{ route('address-types.destroy', $addressType->slug) }}" method="POST" onsubmit="return confirm('Are you sure?');" style="display: inline-block;">
                                                    @csrf
                                                    @method('DELETE')
                    ...
                    <div class="col-xxl-3">
                        <div class="card border card-border-light">
                            <div class="card-header">
                                <h6 id="card-title" class="card-title mb-0">Add  Address Type</h6>
                            </div>
                            <div class="card-body">
                                <form id="edit-form" action="" method="post" enctype="multipart/form-data">
                                    ...
                                    @csrf
                                    <div class="mb-3">
                                        <label for="name" class="form-label">Address Type</label>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('.edit-button').on('click', function() {
                ...
                $('#edit-form').attr('action', '/address-types/' + slug);
                $('input[name="_method"]').val('PATCH');
                submitButton.text('Update');
                // Populate the form for editing
                $('#name').val(name);
                $('#card-title').text('Edit - ' + name + ' Address Type');
                $('#page-title').text('Edit - ' + name + ' Address Type');
            });

            // Click event for adding a new item
            $('#new-button').on('click', function() {
                // Clear the form, set action for creation, method to POST, and button text to Add New
                $('#edit-form').attr('action', '/address-types');
                $('input[name="_method"]').val('POST');
                submitButton.text('Add New');
                $('#name').val('');
                $('#card-title').text('Add Address Type');
                $('#page-title').text('Add New Address Type');
            });
        });
